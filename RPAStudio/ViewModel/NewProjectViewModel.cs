...
namespace RPAStudio.ViewModel
{
    ...
    public class NewProjectViewModel : ViewModelBase
    {
        ...
        {
            XmlDocument doc = new XmlDocument();
            using (var ms = new MemoryStream(RPAStudio.Properties.Resources.ProjectConfig))
            {
                ms.Flush();
                ms.Position = 0;
                doc.Load(ms);
        ...
        private void initProjectUserConfig()
        {
            ...
            {
                defaultCreatePath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"\MoRPA\Projects";
