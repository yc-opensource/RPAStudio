...
namespace RPAStudio.ViewModel
{
    ...
    public class DocumentViewModel : ViewModelBase
    {
        ...
        {
            var settings = ViewModelLocator.Instance.Settings;
            if (_timer == null)
            {
                _timer = new Timer(new TimerCallback(obj =>
                {
                    if (settings.AutoSaveIsEnabled)
                    {
                        Common.RunInUI(SaveDocument);
                        Logger.Debug("执行自动保存操作");
                    }
                }), null, 30 * 1000, settings.AutoSaveSeconds * 1000);
            }
        }
