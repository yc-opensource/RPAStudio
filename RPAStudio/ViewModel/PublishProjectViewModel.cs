...
namespace RPAStudio.ViewModel
{
    ...
    public class PublishProjectViewModel : ViewModelBase
    {
        ...
        private void initRobotDefaults()
        {
            ...
            var packagesDir = commonApplicationData + @"\MoRPA\Packages";//机器人默认读取nupkg包的位置
        ...
        private bool _isOperateBusy;
        public bool IsOperateBusy
        {
            get => _isOperateBusy;
            set
            {
                if (value != _isOperateBusy)
                {
                    _isOperateBusy = value;
                    RaisePropertyChanged(nameof(IsOperateBusy));
                }
            }
        }
                        ...
                        try
                        {
                            ...
                            if (m_nupkgLocation.ToLower().StartsWith("http"))
                            {
                                ...
                                Task.Run(async () =>
                                {
                                    ...
                                    var result = await publishService.Publish(builder, m_nupkgLocation, ViewModelLocator.Instance.Project.ProjectName, publishVersion, ReleaseNotes);
                                    Common.RunInUI(() =>
                                    {
                                        IsOperateBusy = false;
                                        ...
                                        if (result)
                                        {
                                            var messageBoxText2 = string.Format("项目发布成功。\n名称：{0}\n版本：{1}\n位置：{2}\n", ViewModelLocator.Instance.Project.ProjectName, publishVersion, m_nupkgLocation);
                                            MessageBox.Show(Application.Current.MainWindow, messageBoxText2, "提示", MessageBoxButton.OK, MessageBoxImage.Asterisk);
                                            return;
                                        }
                                        var messageBoxText3 = string.Format("项目发布失败！\n名称：{0}\n版本：{1}\n位置：{2}\n", ViewModelLocator.Instance.Project.ProjectName, publishVersion, m_nupkgLocation);
                                        MessageBox.Show(Application.Current.MainWindow, messageBoxText3, "提示", MessageBoxButton.OK, MessageBoxImage.Hand);
                                    });
                                });
                            }
                            else
