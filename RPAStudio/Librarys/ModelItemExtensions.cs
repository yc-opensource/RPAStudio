...
namespace RPAStudio.Librarys
{
    ...
    {
		// Token: 0x0600059A RID: 1434 RVA: 0x00015AB5 File Offset: 0x00013CB5
		...
		{
			return modelItem.ItemType.IsGenericType && modelItem.ItemType.GetGenericTypeDefinition() == typeof(FlowSwitch<>);
		}
		...
		public static bool CanAddItem(this ModelItem modelItem, object item)
		{
			...
			if (item is Activity)
			{
				return modelItem.IsSequence() || modelItem.IsFlowchart();
			}
			if (item is State || item is FinalState)
			{
				return modelItem.IsStateMachine();
			}
			return item is FlowNode && modelItem.IsFlowchart();
		}
		...
		public static ModelItem AddActivity(this ModelItem modelItem, object item, int index = -1)
        {
			...
			{
				State state;
				if ((state = (item as State)) == null)
				{
					(state = new State()).IsFinal = true;
				}
				State value2 = state;
				ModelItemCollection collection3 = modelItem.Properties["States"].Collection;
				result = ((index == -1) ? collection3.Add(value2) : collection3.Insert(index, value2));
			}
			return result;
		}
    }
